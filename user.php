...
# Get passed product id and assign it to a variable.    added 12/10/2023!!!!!!!!!!!!!!
...
$status = "SELECT status FROM users WHERE user_id={$_SESSION[user_id]}";
$rs = mysqli_query( $link, $status ) ;
if ( mysqli_num_rows( $r ) > 0 ) {
  ...
  ';
	...
	<div class="col-sm">
	  ...
	  <p style="color: darkred"><strong> D.O.B : </strong> ' . $row['d_o_b'] . ' </p>
	  <p style="color: darkred"><strong> Contact Number : </strong> ' . $row['c_num'] . ' </p>
	  <p style="color: darkred"><strong> Country : </strong> ' . $row['country'] . ' </p>
	  <p style="color: darkred"><strong> Registration Date : </strong> ' . $row['reg_date'] . ' </p>
	  <p style="color: darkred"><strong> Status : </strong> ' . $row['status'] . ' </p>
	  <br>
	  <br>
	  <br>
	  <hr>
	...
	<button type="button" class="btn btn-outline-danger btn-lg btn-block" data-toggle="modal" data-target="#password">
...
//check status
            if (mysqli_num_rows( $rs ) > 0 ) {
                while ($rows = mysqli_fetch_array($rs, MYSQLI_ASSOC)) {
                    if (($rows['status']) == 'unsubscribed') {
                        echo '
    <a href="paypal.php" role="button" type="button" class="btn btn-outline-primary btn-lg btn-block">Subscription</a>
	';}
	}
	}
	echo'
